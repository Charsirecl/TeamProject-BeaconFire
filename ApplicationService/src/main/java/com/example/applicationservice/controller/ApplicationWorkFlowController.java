...
//    }
    ...
    public ResponseEntity<List<ApplicationWorkFlow>> getApplicationsByEmployeeId(@PathVariable int employeeId) {
        List<ApplicationWorkFlow> applications = applicationWorkFlowService.getApplicationsByEmployeeId(employeeId);
        return ResponseEntity.ok(applications);
    }

    @PostMapping("/submit")
    public ResponseEntity<ApplicationWorkFlow> submitApplication(
            @RequestParam int employeeID,
            @RequestParam String status,
            @RequestParam(required = false) String comment) {

        // Create a new ApplicationWorkFlow object
        ...
        applicationWorkFlow.setComment(comment);

        // Save the application work flow
        ApplicationWorkFlow savedApplication = applicationWorkFlowService.saveApplication(applicationWorkFlow);
